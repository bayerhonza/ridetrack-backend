...
public class SpaceManager {
	...
	@PostFilter("hasPermission(filterObject, T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_READ)")
	public List<Space> findAllSpacesOfClient(Client client) {
		return spaceRepository.findAllByOwner(client);
	}
	
	@PostAuthorize("hasPermission(returnObject.orElse(null), T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_READ)")
	...
	@PreAuthorize("hasPermission(#client, T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_CREATE_SPACE)")
	...
	@PreAuthorize("hasPermission(#space, T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_UPDATE)")
	...
	@PreAuthorize("hasPermission(#client, T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_DELETE_SPACES)")
	...
	@PostAuthorize("hasPermission(returnObject,  T(com.ensimag.ridetrack.auth.privileges.PrivilegeEnum).CAN_READ)")
