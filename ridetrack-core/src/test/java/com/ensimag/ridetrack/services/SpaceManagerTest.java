...
class SpaceManagerTest {
	...
	public void testCreateDefaultSpace() {
		...
		Mockito.when(spaceRepository.findByNameAndOwner(ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Optional.empty());
	...
	public void testCreateSpaceFromName() {
		...
		Client newClient = new Client().toBuilder()
			...
			.build();
		Space newSpace = spaceManager.createSpace("testing-space1",newClient);
	...
	public void testCreateSpaceFromSpace() {
		...
		spaceManager.createSpace(newSpace,newClient);
	...
	public void testCreateExistingSpace() {
		Mockito.when(spaceRepository.findByNameAndOwner(ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Optional.of(new Space()));

		Assertions.assertThrows(RidetrackConflictException.class, () -> spaceManager.createSpace("shall not pass", new Client()));
	}
